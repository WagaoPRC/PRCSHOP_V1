...
namespace NopBrasil.Plugin.Shipping.Correios.Service
{
    public class CorreiosService : ICorreiosService
    {
        ...
        public WSCorreiosCalcPrecoPrazo.cResultado RequestCorreios(GetShippingOptionRequest getShippingOptionRequest)
        {
            ...
            decimal valuePackage = getShippingOptionRequest.Items.Sum(item => item.ShoppingCartItem.Product.Price * item.ShoppingCartItem.Product.StockQuantity);
            //Value in Correios has bigger than R$18
            if (valuePackage < 18m)
                valuePackage = 18m;

            EndpointAddress endpointAddress = new EndpointAddress(_correiosSettings.Url);
            
            Decimal mWeights = GetWheight(getShippingOptionRequest);
            ...
            var ws = wsCorreios.CalcPrecoPrazo(_correiosSettings.CompanyCode, _correiosSettings.Password, GetSelectecServices(_correiosSettings), getShippingOptionRequest.ZipPostalCodeFrom,
                getShippingOptionRequest.ShippingAddress.ZipPostalCode, mWeights.ToString(), 1, length, height, width, 0, "N", valuePackage, "N");

            if (mWeights < 0.300m && _correiosSettings.ServicesOffered.Contains("Carta Registrada"))
            {
                ws.Servicos.Last().Erro = "0";
                ws.Servicos.Last().Valor = "8,00";
                ws.Servicos.Last().PrazoEntrega = "7";
            }
                
            return ws;
        }

        private decimal GetWheight(GetShippingOptionRequest shippingOptionRequest)
        {
           ...
           // Decimal weight = Math.Ceiling(_measureService.ConvertFromPrimaryMeasureWeight(_shippingService.GetTotalWeight(shippingOptionRequest), usedMeasureWeight));
            decimal weight = _measureService.ConvertFromPrimaryMeasureWeight(_shippingService.GetTotalWeight(shippingOptionRequest), usedMeasureWeight);
        ...
        private string GetSelectecServices(CorreiosSettings correioSettings)
        {
            ...
            var v = sb.ToString().Remove(sb.ToString().Length - 1, 1);
